...
#
# The environment
SERVER_IMAGE=santosharakere/jmeter-server
CLIENT_IMAGE=santosharakere/jmeter
DATADIR=
JMX_SCRIPT=
...
function server_ips() {
	...
	for pid in $(docker -H tcp://0.0.0.0:$SWARM_PORT ps | grep ${SERVER_IMAGE} | awk '{print $1}')
	do
...
function wait_for_client() {
	...
	while :
	do
		docker -H tcp://0.0.0.0:$SWARM_PORT ps --no-trunc | grep ${CLIENT_CID} 2>/dev/null 1>&2
		if [[ $? -ne 0 ]]; then
			echo
...
function stop_servers() {
  ...
  n=1
	while [[ ${n} -le ${NUM_SERVERS} ]]
	do
		docker -H tcp://0.0.0.0:$SWARM_PORT stop ${SERVER_NAME_PREFIX}-${n}
		n=$((${n}+1))
	done
...
function remove_containers() {
	...
	while [[ ${n} -le ${NUM_SERVERS} ]]
	do
		docker -H tcp://0.0.0.0:$SWARM_PORT rm ${SERVER_NAME_PREFIX}-${n}
		n=$((${n}+1))
	done
}
